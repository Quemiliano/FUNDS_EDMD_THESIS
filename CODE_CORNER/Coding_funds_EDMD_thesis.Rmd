...
couleurs_1 <- c("#000066", "#000A7A", "#00148F", "#001EA3", "#0028B8", "#0032CC", "#003CE1", "#0046F5", 
                ...
                "#7EA0FF", "#8CAAFF", "#9AB4FF", "#A8BEFF", "#B6C8FF", "#C4D2FF", "#D2DCFF","#FFE490", "#FFE080", "#FFDC70", "#FFD860", "#FFD450", "#FFD040", "#FFCC30", "#FFC820",
...
df_fonds_EDMD= read.csv("/home/emiliano/Documents/MASTER_MAS/ALTERNANCE/RAPPORT APPRENTISSAGE/Partie 2/FONDS EDMD/DATA_PRETREATMENT/fonds_EDMD.csv", sep = ",", dec= ".")
...
# Sélectionner des variables originales basées sur l'ACP


```{r, warning= FALSE, message= FALSE}
# Conversion des colonnes en numérique
df_clean_EDMD_for_PCA <- df_fonds_EDMD %>%
  mutate(across(c(gridens7, gare_tgv, ecoquartiers, climat, beneficiaire_prog, statut_finance_ecolo_dmd), as.numeric)) %>% dplyr:: select(-c("cp_EDMD", "code_com","code_dept","code_arr","lib_com","lib_arr","lib_dept"))
```
...
# Sélectionner les variables ayant une contribution >= seuil_contrib <- 100 / 112
seuil_contrib <- 100 / 112
var_cos2 <- acp$var$cos2
vars_selected <- rownames(var_cos2)[apply(var_cos2, 1, function(x) any(x > 0.5))]

# Visualiser uniquement ces variables

print(fviz_pca_var(acp,
             axes = c(1,2),
             select.var = list(name = vars_selected),
             col.var = "contrib",
...
fviz_pca_ind(acp,
             axes= c(2, 4),
             habillage = as.numeric(df_clean_EDMD_for_PCA$ae_EDMD >0),
             addEllipses = TRUE,
             label = "none",
             palette = "jco",
...
prepare_and_dummify_data = function(data, del_dependance) {
  ...
  data_prepared <- data_EDMD_clean %>%
    mutate(across(where(is.factor), as.character)) %>%
    {
      ...
      factor_vars  <- dplyr::select(., where(function(x) is.character(x) || is.factor(x)))
      ...
      if (ncol(factor_vars) > 0) {
        ...
        bind_cols(numeric_vars, dummy_vars)
      } else {
        # S’il n’y a pas de variables catégorielles, on garde juste les numériques
        numeric_vars
      }
    } %>%
    relocate(cp_EDMD, .after = last_col()) # replacer la cible à la fin
...
#Décommentez ceci pour inclure la ACP dans les donnée 
# df_fonds_EDMD= df_fonds_EDMD %>%
#   dplyr::select(c(data.frame(vars_selected)$vars_selected, cp_EDMD,statut_finance_ecolo_dmd, code_com ))
...
## Regression linéaire simple (commenter ceci pour la partie réduction de dimension car le taux de subvention ne fait pas partie des variables sélectionnées)
...
model_rn$add(layer_dense(units = 1, input_shape = c(118), activation = "linear")) # commenter ceci pour le modele avec réduction de dimension préalable

# model_rn$add(layer_dense(units = 1, input_shape = c(56), activation = "linear")) # Décommenter ceci pour le modele avec réduction de dimension préalable
...
rf_grid <- expand.grid(mtry= c(10, 20, 35, 55), # sqrt(112)≈11, donc autour de cette valeur
                       ...
                       trees= c(500, 1000, 2000, 5000)) # Moins de variation, focus sur valeurs efficaces
...
# rm(rf_resample, best_parameter_rf, foret_finale)
# Ajustement final
foret_finale <- rf_wf %>%
...
model_rn$add(
  layer_dense(
    units = 50,
    input_shape = c(118), # commenter ceci pour le modele avec réduction de dimension préalable
#    input_shape = c(56),# Décommenter ceci pour le modele avec réduction de dimension préalable
    activation = "relu",
...
## Prédiction des financements des communes n'ayant pas demander de financement en 2024(Meilleur modèle avec réduction de dimension )
...
# Définition du modèle KNN (régression)
knn_spec_adj_best <- nearest_neighbor(neighbors = 30,
...
# Workflow
knn_wf_adj_best <- workflow() %>%
  add_model(knn_spec_adj_best) %>%
  add_formula(cp_EDMD ~ .)


# Ajustement direct sans validation croisée
knn_adj_best <- knn_wf_adj_best %>%
  fit(data = df_fonds_EDMD_Finance)

# Prédiction sur les données de test
df_preds_final_EDMD[, "cp_EDMD_preds"] <- predict(knn_adj_best, data_EDMD_NF) 
...
```{r}
var_preds= df_preds_final_EDMD$cp_EDMD_preds

par(mfrow=c(1,2), las=1)

plot(sort(var_preds), col=rev(terrain.colors(length(var_preds))),pch=20, cex=.75, ylab='cp_EDMD_preds')

hist(var_preds, main='', col=rev(terrain.colors(5)), xlab='cp_EDMD_preds')

```


# Cartographie des prédictions du meilleur modèle :
...
```{r}
# df_fonds_EDMD_save= df_fonds_EDMD %>%
#   mutate(across(starts_with(c("ae_", "cp_")), ~ .x * 1000))
# 
# df_fonds_EDMD_save[,"cp_EDMD_preds"]= df_fonds_EDMD_for_carto_pred$cp_EDMD_preds *1000
# df_fonds_EDMD_save[,"cp_EDMD_all"]= df_fonds_EDMD_for_carto_pred$cp_EDMD_all *1000
# 
# df_fonds_EDMD_save= df_fonds_EDMD_save%>%
#   mutate(across(starts_with(c("cp_EDMD_preds", "cp_EDMD_all")), ~ round(.x, 1)))
# df_fonds_EDMD_save  %>%  # Exclure geometry et code_insee
#   write_csv("data_fonds_EDMD_pred.csv")
```